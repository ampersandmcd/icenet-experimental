import os
import torch
from diffusers import AutoencoderKL, UNet2DConditionModel, PNDMScheduler, DPMSolverMultistepScheduler, DDPMScheduler
...
class TrainingConfig:
    ...
    output_dir = "results/nonlatent-train-93-only"  # the model name locally and on the HF Hub
    ...
    with torch.autocast(device_type="cuda"):
        ...
        for step, batch in enumerate(val_dl):
            ...
            with torch.no_grad():
                ...
                pred = unet(previous_ice, 0, encoder_hidden_states=encoder_hidden_states).sample
                outputs = pred + previous_ice
...
def train():
    ...
    with torch.autocast(device_type="cuda"):
        ...
        unet = UNet2DConditionModel(
            ...
            block_out_channels = (16, 32, 64, 128), # (4, 8, 16, 32),
        ...
        conditioner = AutoencoderKL(
            ...
            block_out_channels = (16, 16, 16, 16, 16, 32, 64, 128),
            latent_channels=config.cross_attention_dim,
            norm_num_groups=1
        )
        print(f"UNet Parameters: {sum([p.numel() for p in unet.parameters()])}")
        print(f"Conditioner Parameters: {sum([p.numel() for p in conditioner.encoder.parameters()])}")
        ...
        for epoch in range(config.num_epochs):
            ...
            for step, batch in enumerate(train_dl):
                ...
                with accelerator.accumulate(unet, conditioner):
                    ...
                    pred = unet(previous_ice, timesteps, encoder_hidden_states=conditioner_hidden_states).sample

                    # compute mse loss on residual (training u-net)
                    denoising_loss = F.mse_loss(pred, targets)
                ...
                if (global_step + 1) % len(train_dl) == 0:
                    validate(config, global_step, unet, conditioner, scheduler, val_dl, accelerator)
