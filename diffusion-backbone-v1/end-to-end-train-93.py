...
class TrainingConfig:
    ...
    latent_size = 54
    latent_channels = 93  # TODO: try more latent channels for longer forecasts
    in_channels = 26
    mid_channels = 128
    out_channels = 93
    train_batch_size = 1
...
def train():
    ...
    with torch.autocast(device_type="cuda"):
        ...
        unet = UNet2DConditionModel(
            in_channels = config.latent_channels,  # from pure noise
        ...
        ice_ae = AutoencoderKL(
            ...
            block_out_channels = (93, 93, 93, 93),  # (8, 16, 32, 64),
            norm_num_groups=1,
        ...
        for epoch in range(config.num_epochs):
            ...
            for step, batch in enumerate(train_dl):
                ...
                with accelerator.accumulate(unet, condition_ae, ice_ae):
                    ...
                    # compute reconstruction loss from encoding-latent-decoding process on ice (training vae)
                    if config.use_mask:
                    ...
                    # approximate one-step solve to couple u-net and vae training
                    scheduler.set_timesteps(1)
                    latents = scheduler.step(noise_pred, torch.ones_like(timesteps), noisy_images).prev_sample
                    outputs = ice_ae.decode(latents).sample
                    coupled_loss = F.mse_loss(outputs, targets)

                    # sum and backprop
                    loss = denoising_loss + config.reconstruction_factor * reconstruction_loss + config.coupled_factor * coupled_loss
                ...
                logs = {
                    ...
                    "coupled_loss": coupled_loss.detach().item(),
