...
class LitDiffusion(pl.LightningModule):
    ...
    def rollout(self, x, sample_weight, start_date, end_date):
        ...
        for date in tqdm(date_range):
            ...
            noisy_y = torch.where(noisy_y < self.min_sic_threshold, 0, noisy_y)  # clip residual noise
            ...
            x = torch.cat((
                noisy_y,   # set ice at t+1 to ice at t
...
#     def forward(self, condition_x):
#         """
#         Implement forward function. Sets timestep=0 in embedding.
#         :param condition_x: Conditioning information.
#         :return: Outputs of model.
#         """
#         return self.model(condition_x, 0).sample  # (bs, target_c, resolution, resolution)
...
#         if self.forecast_residuals:
#             y = y + x[:, [0], :, :]  # add on most recent ice condition (b, t, h, w)
#             noisy_y = noisy_y + x[:, [0], :, :]
...
#         fig, ax = plt.subplots(nrows, ncols, figsize=(5*ncols, 5*nrows))
#         for i in range(nrows):
#             ax[i][0].imshow(samples[i].squeeze().cpu(), vmin=0, vmax=1)
#             ax[i][0].set_title(f"Prediction for Condition {i}")
#             ax[i][1].imshow(y[i].squeeze().cpu(), vmin=0, vmax=1)
#             ax[i][1].set_title(f"Truth for Condition {i}")
#             ax[i][2].imshow(diffs[i].squeeze().cpu(), vmin=-1, vmax=1, cmap="bwr")
#             ax[i][2].set_title(f"(Truth - UNet) for Condition {i}")
#         plt.tight_layout()
#         self.logger.experiment.log({"Sample": plt})

#         # need to have a think on how to compute metrics across sample/mean
#         val_mse = F.mse_loss(samples, y)
#         self.log("val_mse", val_mse, prog_bar=True)
...
#         self.noise_scheduler.set_timesteps(self.num_train_timesteps)
#         dt = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
#         # torch.save(self.model, f"results/lightning-unet-epoch-{self.current_epoch}-{dt}.pth")
...
def train_icenet(args):
    ...
    if args.model == "diffusion":
        if args.scheduler == "ddpm":
            noise_scheduler = DDPMScheduler(num_train_timesteps=args.scheduler_timesteps, prediction_type=args.prediction_type, beta_schedule="squaredcos_cap_v2")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--scheduler", default="ddim", type=str, choices=["ddpm", "ddim", "heun"], help="Noise scheduler strategy")
    parser.add_argument("--scheduler_timesteps", default=20, type=int, help="Number of noising timesteps")
    parser.add_argument("--prediction_type", default="v_prediction", type=str, choices=["epsilon", "v_prediction", "sample"], help="Scheduler prediction type")
