...
from utils import IceNetDataSetPyTorch, BinaryAccuracy, SIEError, SIAError, RMSError, MAError
...
def validate(config, epoch, unet, condition_ae, ice_ae, scheduler, val_dl, accelerator):
    ...
    metrics = {
        "val_rmse": RMSError(leadtimes_to_evaluate=list(range(config.out_channels))),
        "val_mae": MAError(leadtimes_to_evaluate=list(range(config.out_channels))),
        "val_accuracy": BinaryAccuracy(leadtimes_to_evaluate=list(range(config.out_channels))),
        "val_sieerror": SIEError(leadtimes_to_evaluate=list(range(config.out_channels))),
        "val_siaerror": SIAError(leadtimes_to_evaluate=list(range(config.out_channels)))
    }
    ...
    accelerator.log_dict(metrics.compute())  # epoch-level metrics
    metrics.reset()

    # make plot of selected inputs/outputs
    fig, ax = plt.subplots(4, 5, figsize=(25, 20))
    mat_in = x.squeeze().detach().cpu().numpy()
    mat_out = outputs.squeeze().detach().cpu().numpy()
    mat_true = y.squeeze().detach().cpu().numpy()
    ax[0][0].imshow(mat_in[2])
    ax[0][0].set_title("Initial")
    for j, timestep in enumerate([0, 30, 60, 90]):
        ax[0][j+1].imshow(mat_true[timestep])
        ax[0][j+1].imshow(f"True @ t={timestep}")
        ax[1][j+1].imshow(mat_out[timestep])
        ax[1][j+1].imshow(f"Forecast @ t={timestep}")
        ax[2][j+1].imshow(mat_out[timestep] - mat_true[timestep])
        ax[2][j+1].imshow(f"Forecast - Truth @ t={timestep}")
        ax[3][j+1].imshow(mat_out[timestep] - mat_in[2])
        ax[3][j+1].imshow(f"Forecast - Initial @ t={timestep}")
    plt.tight_layout()
...
def train():
    ...
    val_ds = IceNetDataSetPyTorch(config.dataset_config, "val", batch_size=config.eval_batch_size, shuffling=False)
